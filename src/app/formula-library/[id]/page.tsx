...
export default function FormulaLibrary({ params }: { params: { id: string } }) {
    ...
    const getFormulaData = async () => {
        try {
            ...
            const formula = await fetch('/api/formulas', {
                method: 'POST',
                body: JSON.stringify(payload),
            });

            if (!formula.ok) throw new Error('Error getting tone of voice');

            const formulaData = await formula.json();

            console.log(formulaData)

            if (!!formulaData.formula.title) setTitle(formulaData.formula.title);
            if (!!formulaData.formula.instructions) setInstructions(formulaData.formula.instructions);
            if (!!formulaData.formula.formula) setFormula(formulaData.formula.formula);
