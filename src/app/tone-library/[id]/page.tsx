...
function Page({ params }: { params: { id: string } }) {
    ...
    const getToneData = async () => {
        try {
            const tone = await fetch('/api/tonesGetSingle', {
                method: 'POST',
    ...
    /* Delete the tone
    /* * * * * * * * * * * * * * * * */
    const handleDeleteTone = async (index: number) => {
        try {
            const payload = {
                'dataType': 'delete',
                'data': params.id,
    ...
    return (
        <section className="relative min-h-screen">
            <div className="grid grid-cols-5 w-full min-h-screen max-w-[90%] mx-auto bg-gray-200/50 rounded-t-3xl p-4 gap-4">
                <div className="col-span-5 flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-4">
                        {!!examples.length && (
                            <button className="bg-theme_primary-500 text-white py-1 px-2 rounded-md max-w-max ml-auto" onClick={() => setDisplayAddExample(!displayAddExample)}>{displayAddExample ? 'Close' : 'Add new example'}</button>
                        )}
                        {!examples.length && (
                            <form className="flex flex-col gap-2 p-4 bg-gray-200 border-4 border-gray-400/50 rounded-md border-dashed" onSubmit={(e) => { e.preventDefault(); handleAddExample(newExample); }}>
                                ...
                                <button type="submit" className="ml-auto bg-theme_primary-500 text-white py-1 px-2 rounded-md max-w-max ml-auto">Add example</button>
                            </form>
                        )}
                        {
                            !!displayAddExample && !!examples.length && (
                                <form className="flex flex-col gap-4 p-4 border-4 border-gray-400/50 rounded-md border-dashed bg-gray-200" onSubmit={(e) => { e.preventDefault(); handleAddExample(newExample); }}>
                                    ...
                                    <button type="submit" className="ml-auto bg-theme_primary-500 text-white py-1 px-2 rounded-md max-w-max">Add example</button>
                                </form>
                            )
                        }
                        {!!examples.length && (
            ...
            <div className="sticky bottom-0 right-0 max-w-[88%] mx-auto bg-white rounded-t-3xl flex gap-4 p-4 shadow-[0_-5px_15px_-15px_rgba(0,0,0,0.6)]">
                ...
                <button className="bg-theme_primary-500 text-white py-1 px-2 rounded-md max-w-max" onClick={handleUpdateTone}>Update</button>
