      ...
      if (selectedFiles && selectedFiles.length > 0) {
        ...
        const uploadedFiles = await Promise.all(
          Array.from(selectedFiles).map(async (file) => {
            ...
            if (
              file.type.match(
                /(text\/plain|application\/(pdf|msword|vnd\.openxmlformats-officedocument\.wordprocessingml\.document|vnd\.openxmlformats-officedocument\.presentationml\.presentation))/
              ...
              alert(
                `Invalid file type or size. Only TXT, PD, PPTX or DOCX are allowed, up to ${props.maxFileSizeMB}MB.`
              );
              return null; // Skip this file
            }
          })
...
// Create a function that fires on app load that uses axios to get data from the "files" endpoint and logs the data to the console
const getFiles = useCallback(async () => {
  try {
    const response = await axios.get(`${SERVER_ADDRESS}/files`);
	...
	<div className="flex items-center justify-center w-full flex-col">
		...
		>
			<div className="flex flex-col items-center justify-center pt-5 pb-6">
				{loading ? (
					<LoadingText text="Uploading..." className={''} iconClassName={''} />
				) : (
					<div className="text-gray-500 flex flex-col items-center text-center">
						...
						<p className="text-xs">
							PDF, PPTX, DOCX or TXT (max {props.maxFileSizeMB}MB per file)
						</p>
